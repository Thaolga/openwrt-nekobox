...
if nxfs.access('/etc/config/spectra') then
	...
	bing_background = uci:get_first('spectra', 'global', 'bing_background')
end
...
-- [[ 模糊设置 ]]--
br = SimpleForm('config', translate('Spectra Config'), translate('Here you can set the blur and transparency of the login page of spectra theme, and manage the background pictures and videos.[Chrome is recommended]'))
br.reset = false
br.submit = false
...
function br.handle(self, state, data)
    if (state == FORM_VALID and data.blur ~= nil and data.blur_dark ~= nil and data.transparency ~= nil and data.transparency_dark ~= nil and data.mode ~= nil) then
        ...
        for key, value in pairs(data) do
            uci:set('spectra','@global[0]',key,value)
        end 
        uci:commit('spectra')
    end
    return true
